...
/// (2) when exchanging coins, we call `exchange_to` to swap `AptosCoin` to `CanCoin`, and `exchange_from` to swap `AptosCoin` from `CanCoin`
module cannedbi::can_defi {
    use std::signer;
    use std::string;
    ...
    struct ModuleData has key {
        ...
        burn_cap: BurnCapability<CanCoin>,
        mint_cap: MintCapability<CanCoin>,
    }
    ...
    fun init_module(account: &signer) {
        ...
        let (burn_cap, freeze_cap, mint_cap) = coin::initialize<CanCoin>(
            account,
            string::utf8(b"Cannedbi Coin"),
            string::utf8(b"CAN"),
            8,
            false,
        );
        ...
        coin::register<CanCoin>(account);
    }

    /// Exchange AptosCoin to CanCoin
    public fun exchange_to(a_coin: Coin<AptosCoin>): Coin<CanCoin> acquires ModuleData {
        let coin_cap = borrow_global_mut<ModuleData>(@cannedbi);
        let amount = coin::value(&a_coin);
        coin::deposit(@cannedbi, a_coin);
        coin::mint<CanCoin>(amount, &coin_cap.mint_cap)
    }

    /// Exchange CanCoin to AptosCoin
    public fun exchange_from(c_coin: Coin<CanCoin>): Coin<AptosCoin> acquires ModuleData {
        let amount = coin::value(&c_coin);
        let coin_cap = borrow_global_mut<ModuleData>(@cannedbi);
        coin::burn<CanCoin>(c_coin, &coin_cap.burn_cap);

        let module_data = borrow_global_mut<ModuleData>(@cannedbi);
    ...
    public entry fun exchange_from_entry(account: &signer, amount: u64) acquires ModuleData {
        let c_coin = coin::withdraw<CanCoin>(account, amount);
    ...
    public entry fun test_exchange_to_and_exchange_from(origin_account: signer, resource_account: signer, framework: signer) acquires ModuleData {
        ...
        assert!(coin::balance<CanCoin>(signer::address_of(&resource_account)) == 0, 2);
        ...
        assert!(coin::balance<CanCoin>(signer::address_of(&resource_account)) == 0, 4);
