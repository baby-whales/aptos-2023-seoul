...
namespace Mirage.Aptos.SDK
{
    public static class WebHelper
    {
        ...
        public static Task<TResultType> SendPostRequest<TPayloadType, TResultType>(string url, TPayloadType payload, Dictionary<string, string> headers = null, Dictionary<string, string> query = null)
        {
        ...
        public static Task<TResultType> SendPostRequest<TResultType>(string url, Dictionary<string, string> headers = null, Dictionary<string, string> query = null)
        {
            return SendChangeRequest<TResultType>(url, string.Empty, headers, query);
        }

        public static async Task<TResultType> SendGetRequest<TResultType>(string url, Dictionary<string, string> headers = null, Dictionary<string, string> query = null, string wrapper = null)
        {
            using (var request = new HttpClient())
            {
                ...
                return await SendGetRequestUniTask<TResultType>(url);
            }
        }

        private static async UniTask<TResultType> SendGetRequestUniTask<TResultType>(string url)
        {
            var result = (await UnityEngine.Networking.UnityWebRequest.Get(url).SendWebRequest()).downloadHandler.text;

            return JsonConvert.DeserializeObject<TResultType>(result);
        }

        private static async Task<TResultType> SendChangeRequest<TResultType>(string url, string payload, Dictionary<string, string> headers = null, Dictionary<string, string> query = null, string wrapper = null)
        {
